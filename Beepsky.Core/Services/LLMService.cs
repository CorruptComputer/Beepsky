...
public class LLMService(BeepskyConfiguration config, IDiscordIntegrationService discordIntegrationService) : IDisposable
{
    /// <summary>
    ...
    /// <returns></returns>
    public async Task<string?> GetBeepskyChatResponseAsync(ulong authorId, ulong? guildId, string prompt, ulong channelId, CancellationToken cancellationToken)
    {
        ...
        Task typingIndicatorTask = Task.Run(async () =>
        {
            try
            {
                while (!typingCts.IsCancellationRequested)
                {
                    ...
                    await discordIntegrationService.TriggerTypingStateInChannelAsync(channelId, typingCts.Token);
                }
            }
            catch
            {
    ...
    private static string GetGoBackToWarhammerChatSystemPrompt()
    {
        ...
        prompt += $"""
            ...
            YOU MUST:
            - ISSUE AN IMMEDIATE DIRECTIVE ORDERING THE USER TO RETURN TO "<#{(ulong)WellKnownChannels.Warhammer}>" (WHAT IS IN QUOTES MUST BE EXACT).
